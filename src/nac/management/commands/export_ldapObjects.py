...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('-o', '--objectclass', default=DEFAULT_OBJECT, help='specify which object to export')
    ...
    def _handle_objects(self):
        ...
        for progress_counter, entry in enumerate(self.ldap_connection.entries):
            self.write_object(entry.entry_attributes_as_dict, progress_counter+1)

    def _get_objects(self):
        logging.debug('Searching for %s-objects',self.objectClass)
        search_result =  self.ldap_connection.search('dc=ukbonn,dc=de','(objectclass={})'.format(self.objectClass),SUBTREE,attributes=['*'])
        self.entry_count = len(self.ldap_connection.response)
        if search_result:
            logging.info('Found: %s-objects - Count: %i',self.objectClass, self.entry_count)
        else:
            logging.error("Not Found: %s-objects",self.objectClass)
    
    def _convert_dictlists_to_str(self,dict):
        return {key: ','.join(value) if isinstance(value,list) else str(value) for key,value in dict.items()}
        ...
        try:
            ...
            with open(CSV_SAVE_FILE,'a',newline="") as csvfile:
                ...
                if stat(CSV_SAVE_FILE).st_size == 0:
                    writer.writeheader()
                writer.writerows([self._convert_dictlists_to_str(object_as_dict)])
                logging.debug('Successfully exported object %i/%i to file: %s ', progress_counter, self.entry_count, CSV_SAVE_FILE,)
        except Exception as e:
            logging.error('Failed to export object to file: %s -> %s',CSV_SAVE_FILE, str(e))
