import pytest
from nac.management.commands.export_ldapObjects import Command, CSV_SAVE_FILE, DEFAULT_OBJECT, DEFAULT_CONFIG
...
def test_add_arguments(command):
    ...
    mock_parser.add_argument.assert_any_call('-c', '--config_file', default=DEFAULT_CONFIG, help='use a specific config file [src/export.cnf]')
    mock_parser.add_argument.assert_any_call('-o', '--objectclass', default=DEFAULT_OBJECT, help='specify which object to export')
...
def test_handle(mock_handle_objects, mock_get_objects, mock_clear_file, 
                mock_connect_to_ldap, mock_get_config, mock_setup_logger, command):
    options = {
        'verbosity': 0,
...
def test_get_objects_found(mock_logging, command):
    ...
    command.ldap_connection.search.assert_called_once_with('dc=ukbonn,dc=de', '(objectclass=testObject)', SUBTREE, attributes=['*'])
    mock_logging.info.assert_called_once_with('Found: %s-objects - Count: %i', 'testObject', 2)
...
def test_get_objects_not_found(mock_logging, command):
    ...
    command.ldap_connection.search.assert_called_once_with('dc=ukbonn,dc=de', '(objectclass=testObject)', SUBTREE, attributes=['*'])
    mock_logging.error.assert_called_once_with('Not Found: %s-objects','testObject')
...
def test_clear_file_exception(mock_logging, mock_file, command):
    command._clear_file()
    mock_logging.error.assert_called_once_with('Failed to truncate %s -> %s', CSV_SAVE_FILE, 'Failed')
...
@patch('nac.management.commands.export_ldapObjects.logging')
def test_write_object_empty_file(mock_logging, mock_stat, mock_writer, mock_file, command):
    ...
    mock_writer.assert_called_once_with(mock_file(), fieldnames=object_dict.keys(), delimiter=';')
    mock_writer_instance.writeheader.assert_called_once()
    mock_writer_instance.writerows.assert_called_once_with([{'attr1': 'value1', 'attr2': 'value2,value3'}])
    mock_logging.debug.assert_called_once_with('Successfully exported object %i/%i to file: %s ', 5, 10, CSV_SAVE_FILE)
...
@patch('nac.management.commands.export_ldapObjects.stat')
@patch('nac.management.commands.export_ldapObjects.logging')
def test_write_object_non_empty_file(mock_logging, mock_stat, mock_writer, mock_file, command):
    object_dict = {'attr1': 'value1', 'attr2': ['value2', 'value3']}
...
def test_write_object_exception(mock_logging, mock_file, command):
    ...
    mock_logging.error.assert_called_once_with('Failed to export object to file: %s -> %s', CSV_SAVE_FILE, 'Failed')